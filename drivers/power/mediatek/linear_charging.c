...
PMU_STATUS do_jeita_state_machine(void)
{
...
#ifdef HIGH_BATTERY_VOLTAGE_SUPPORT
	#if defined(CONFIG_PROJECT_S5400) || defined(CONFIG_PROJECT_S4800)
	...
	//yixuhong 20150128 delete if (g_temp_status != previous_g_temp_status) 
	{
...
static void set_jeita_charging_current(void)
{
	...
	g_temp_CC_value = JEITA_TEMP_POS_45_TO_POS_60_CC_CURRENT;
        ...
        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] JEITA set charging current zx1 : %d,status=%d\r\n", g_temp_CC_value,BMT_status.charger_type);		
        }
        else if(g_temp_status == TEMP_POS_0_TO_POS_10)
        {
                g_temp_CC_value = JEITA_TEMP_POS_0_TO_POS_10_CC_CURRENT;
                battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] JEITA set charging current zx2: %d\r\n", g_temp_CC_value);		
        }	
		#if defined(CONFIG_PROJECT_S5400)
		else if (g_temp_status == TEMP_NEG_10_TO_POS_0)
		{
                g_temp_CC_value = JEITA_TEMP_NEG_10_TO_POS_0_CC_CURRENT;
                battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] TEMP_NEG_10_TO_POS_0 set charging current: %d\r\n", g_temp_CC_value);		
        }
		#endif	
        else
        {
	g_temp_CC_value = CHARGE_CURRENT_0_00_MA;
      ...
      #endif
battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] JEITA set charging current zx3: temp=%d,type=%d,current=%d\r\n",g_temp_status,BMT_status.charger_type, g_temp_CC_value);		
}
...
void select_charging_curret(void)
{
	if (g_ftm_battery_flag) {
		...
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Default CC mode charging zx: %d\r\n",
				    g_temp_CC_value);
